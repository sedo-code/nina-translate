...
def create_desktop_qr(url):
    desktop = os.path.join(os.environ['USERPROFILE'], 'Desktop')
    html_path = os.path.join(desktop, 'Mobil-Ceviri-Baglantisi.html')
    html_content = f"""<!DOCTYPE html>
<html>
<head>
    <meta charset="utf-8">
    <title>Mobil Ceviri Baglantisi</title>
    <style>
        body {{ font-family: sans-serif; background: #071421; color: #fff; text-align: center; padding: 40px; }}
        .card {{ background: rgba(8, 19, 31, 0.92); border: 1px solid rgba(255,255,255,0.12); padding: 30px; border-radius: 20px; display: inline-block; box-shadow: 0 20px 45px rgba(0,0,0,0.35); }}
        h1 {{ font-size: 22px; margin-bottom: 10px; }}
        p {{ color: #b8c2cf; margin-bottom: 20px; font-size: 14px; }}
        img {{ background: #fff; padding: 15px; border-radius: 10px; width: 200px; height: 200px; }}
        a {{ color: #3aa8ff; text-decoration: none; font-weight: bold; font-size: 16px; display: block; margin-top: 20px; word-break: break-all; }}
    </style>
</head>
<body>
    <div class="card">
        <h1>📱 Mobil Çeviri Bağlantısı</h1>
        <p>Bu QR kodu telefonunuzun kamerasıyla taratarak<br>simültane çeviri arayüzünü mobilde açabilirsiniz.</p>
...
def start_tunnel():
    ...
    err_path = os.path.join(base_dir, "tunnel.err")
    ...
    try:
        # Kill any existing ssh processes first
        ...
        url = None
        # Wait for the tunnel URL to appear in the log file (up to 120 seconds)
        for _ in range(240):
            time.sleep(0.5)
            if os.path.exists(log_path):
                try:
                    with open(log_path, "r", encoding="utf-8") as f:
                        content = f.read()
                    match = re.search(r'https://[a-zA-Z0-9.-]+\.(life|localhost\.run)', content)
                    if match:
                        url = match.group(0)
                        create_desktop_qr(url)
                        webbrowser.open(url)
                        break
                except Exception:
                    pass
